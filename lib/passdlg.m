function Answer=passdlg
%PASSDLG Username and password dialog box.
%  ANSWER = INPUTDLG creates a modal dialog box that returns user
%  input for multiple prompts in the cell array ANSWER. 
%
%  This is an edited version of the inputdlg built in function. This is not
%  a particularly secure system -- so don't use it for anything that needs
%  to be ultra secure! It's just so that when you're typing with someone
%  next to you they're not seeing your NetID password
%
%  passdlg uses UIWAIT to suspend execution until the user responds.
%
%
% Since this version is only used for a username and password, it (currently)
% doesn't need any inputs
% 
% KLB September 2018
...
% --- The edited portion that will make it so we only have 2 inputs, and
% the second is typed as '*****'
for lp=1:NumQuest
  if ~ischar(DefAns{lp})
    delete(InputFig);
